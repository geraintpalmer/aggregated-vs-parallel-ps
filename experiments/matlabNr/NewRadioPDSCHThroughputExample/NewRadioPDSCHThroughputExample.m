...
simParameters.NFrames = NFramesScript;      % Number of 10 ms frames
simParameters.SNRIn = [-5 0 5]; % SNR range (dB)
simParameters.SNRIn = minSNRIn:maxSNRIn; % SNR range (dB)
...
simParameters.Carrier.NSizeGrid = 138;           % Bandwidth in number of resource blocks (52 RBs at 15 kHz SCS for 10 MHz BW)
simParameters.Carrier.NSizeGrid = NSizeGridScript;
simParameters.Carrier.SubcarrierSpacing = 30;    % 15, 30, 60, 120 (kHz)
simParameters.Carrier.SubcarrierSpacing = 60;   % 15, 30, 60, 120 (kHz)
simParameters.Carrier.SubcarrierSpacing = SubcarrierSpacingScript;
...
if simParameters.PDSCH.NumCodewords > 1                             % Multicodeword transmission (when number of layers being > 4)
    simParameters.PDSCH.Modulation = {'256QAM','256QAM'};             % 'QPSK', '16QAM', '64QAM', '256QAM'
    simParameters.PDSCH.Modulation = {MCSScript,MCSScript};
    simParameters.PDSCHExtension.TargetCodeRate = [490 490]/1024;   % Code rate used to calculate transport block sizes
else
    simParameters.PDSCH.Modulation = MCSScript;                       % 'QPSK', '16QAM', '64QAM', '256QAM'
...
simParameters.PDSCHExtension.NHARQProcesses = 1;    % Number of parallel HARQ processes to use
...
if contains(simParameters.DelayProfile,'CDL','IgnoreCase',true)
    ...
    % Set vehicular carrier freq
    channel.CarrierFrequency = 4.8e9; % 4.8GHz
...
% set the 'displaySimulationInformation' variable above to false
    ...
    % Keep track of the number of number of repetitions
    reps = 0;
    success_reps = [];
    ...
    for nslot = 0:NSlots-1
        ...
        % show if new data or it is a Rtx
        % fprintf("\n  New data: %d\n", harqEntity.NewData);
        ...
        % In case of success store the required repetitions
        if contains(procstatus,'passed','IgnoreCase',true)
            success_reps(length(success_reps)+1) = reps;
        end
     end
    ...
    fprintf('\nMaximum repetitions: %d', max(success_reps));
end
