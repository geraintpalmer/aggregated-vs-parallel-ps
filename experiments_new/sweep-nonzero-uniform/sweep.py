...
def write_cdf(R, rho):
    ...
    file_path = f"nonzero-uniform-R={R}-rho={round(rho, 2)}.csv"
    S = jsq.SimulationUniform(lambda_=rho * R, mu=1, R=R, max_time=160000, warmup=8000, ps_bar=False, nonzero=True)
    S.run(0)
